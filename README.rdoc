...
3. Creating Postgres users & extensions - We will create a user (geo_rails) with superuser, createdb permissions:

* $$ sudo su postgres $$ psql > create role sms with SUPERUSER CREATEDB LOGIN password '123456';

4. Run the following commands in project directory:
...
5. To populate sample data, run the following rake task:
